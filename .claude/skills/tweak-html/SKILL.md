---
name: tweak-html
description: Inject a live tweak panel into any single-file HTML output (websites, brand book HTML, editorial layouts, mockups). Auto-scans the page, picks reasonable sliders, lets the user dial in changes in the browser, then bakes the values back into the source CSS and removes the panel. Two levels — light (5 sliders), max (10 sliders). Different from master-slides /tweak [N] which is for slide-deck adjustments.
---
...
> **Namespace note:** Renamed from upstream `tweak` to `tweak-html` to avoid trigger collision with `master-slides`'s `/tweak [N]` (which adjusts slide N in a master-slides deck). Use `/tweak-html [file]` here. The light/max levels and bake/strip workflow are unchanged from upstream (robonuggets/skills, 2026-05-11).

Drop a live controls panel into any single-file HTML output, let the user dial in changes in the browser, then bake the values back into the source.

## Triggers

- `/tweak-html [file]` → default level (light, 5 sliders)
- `/tweak-html light [file]` → 5 universal sliders
- `/tweak-html max [file]` → 10 sliders with finer-grained targets
- `tweak this html` / `add tweak panel to this html` (when a file is open or referenced)
- `bake tweak-html` / `tweak-html bake` → apply the user's dialed values back to source + remove panel
- `tweak-html strip` → remove panel without baking (revert)

(Bare `tweak this` may go to either skill; use the `-html` suffix when working on standalone HTML files to disambiguate from master-slides.)
