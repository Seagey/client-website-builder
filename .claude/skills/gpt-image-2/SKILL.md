...
## House rules (Underseage)

Apply these every time this skill is used, regardless of the caller.

### 1. Key loading
`FAL_KEY` lives at the workspace root `.env` (owner-only perms). Agents run from varying cwds, so load it by absolute path before any curl/node/python call:

```bash
export FAL_KEY=$(grep '^FAL_KEY=' "/Users/joshuaseage/Downloads/Underseage.studio projects/Claude Agents/.env" | cut -d= -f2-)
```

Never print the key in logs, tool output, or commits.

### 2. Quality defaults (cost discipline)
- **Medium** by default — first-pass and iteration. ~$0.05/image.
- **Low** when running many concepts in parallel (e.g. the 5×5 grid pattern below). ~$0.02/image.
- **High** only when Josh explicitly says "this is the final" or "bump to high". ~$0.18/image.

Never promote to `high` on your own initiative — ask first if you're unsure.

### 3. 5×5 grid for concept exploration
Whenever Josh asks for "multiple concepts / variants / options" without a specific count, default to generating **one composite image containing a 5×5 grid of 25 variants**, each cell numbered 1–25 in a **small white circle** in a consistent corner (top-left) so cells can be cropped and isolated later.

This is a single API call producing one image with an internal grid — not 25 separate calls. Favor `low` quality for grid runs since each cell is small. Pattern into the prompt:

> "A 5×5 grid (25 cells, 5 rows × 5 columns) of variants for {subject}. Each cell shows a distinct variation. Label each cell with a small white filled circle in the top-left corner containing its number 1–25 in clean black sans-serif. Cells separated by thin white gutters. No outer frame or title. Each variant should meaningfully differ in {lever — e.g. color palette, layout, typography style, composition}."

After generation, Josh picks cell numbers; regenerate those at higher quality as finals.

### 4. Priority use case — social media
Primary demand is content that **requires legible words** (captions, headlines, callouts, chalk-style posts). Default to formats that work natively on social: `square_hd` (1:1) for IG feed, `portrait_16_9` for Stories/Reels, `landscape_16_9` for YouTube/X.

### 5. Save outputs to the central RUBRIC Generations gallery
Fal URLs expire. Always download `images[i].url` to disk. **Default output directory: `/Users/joshuaseage/Downloads/Underseage.studio projects/Claude Agents/dashboard/generations/generations/`** so the result appears in the RUBRIC Console Generations tab on port 5050. Filename `{label}_{ts}.{ext}`. If the user asks for a different location (e.g. an in-project asset path), copy there as well — but always also save the canonical copy in the central gallery.
...
async function gptImage2({
  ...
  num_images = 1,
  output_format = 'png',
  outDir = '/Users/joshuaseage/Downloads/Underseage.studio projects/Claude Agents/dashboard/generations/generations',
  label = 'gpt2',
}) {
